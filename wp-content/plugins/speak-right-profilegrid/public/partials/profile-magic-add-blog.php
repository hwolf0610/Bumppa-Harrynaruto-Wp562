...
if(isset($_POST['blog_title']))
{
    ...
    if (!wp_verify_nonce($retrieved_nonce, 'pg_blog_post' ) ) die( __('Failed security check','profilegrid-user-profiles-groups-and-communities') );
    ...
    if($postid)
    {
        ...
        {
            _e('Your blog post has been published successfully!','profilegrid-user-profiles-groups-and-communities');
        }
        else
        {
            _e('Your blog post has been submitted successfully and is pending approval. Once approved, it will start appearing in your profile.','profilegrid-user-profiles-groups-and-communities');
        }
        echo '</div>';
        
        if($pm_blog_notification_admin=="1")
        {
           $subject = __('New User Blog Post Submitted','profilegrid-user-profiles-groups-and-communities');
    ...
    {
        echo '<div class="pm_blog_post_message">';
            _e("Something went wrong. Your blog post wasn't published. Please try again or contact the admin.",'profilegrid-user-profiles-groups-and-communities');
        echo '</div>';        
    }
    $redirect_url = esc_url_raw($pmrequests->profile_magic_get_frontend_url('pm_user_profile_page',site_url('/wp-login.php')));
    
    echo '<div class="pm_blog_post_message">';
           echo '<a href="'.$redirect_url.'">'. __("Go to My Profile",'profilegrid-user-profiles-groups-and-communities').'</a>';
        echo '</div>';
