...
if( !function_exists( 'multiple_business_enqueue' ) ):
/**
...
* @return void
*/
function multiple_business_enqueue( $scripts ){

    # Do not enqueue anything if no array is supplied.
    if( ! is_array( $scripts ) ) return;

    $scripts = apply_filters( 'multiple_business_scripts' , $scripts );
...
if( ! function_exists( 'multiple_business_get_related_posts' ) ):
/**
* Get related posts
*
* @since Multiple Business 1.0.0
...
* 
*/
function multiple_business_get_related_posts( $taxonomy = array(), $per_page = 3, $get_params = false ){
...
* @return string	
*/	
function multiple_business_modify_post_navigation_markup( $template, $class ){
...
add_filter( 'navigation_markup_template', 'multiple_business_modify_post_navigation_markup', 10, 2 );

if( ! function_exists( 'multiple_business_primary_menu_fb' ) ) :
/**
 ...
 * @return void
 */
function multiple_business_primary_menu_fb( $arg ) {
    if( !$arg[ 'echo' ] ){
        ob_start();
    }
    ?>
    <ul>
        <li>
            <a href="<?php echo esc_url( home_url( '/' ) ) ?>"><?php esc_html_e( 'Home', 'multiple-business' ) ?></a>
        </li>
    </ul>
    <?php
    if( !$arg[ 'echo' ] ){
...
if( !function_exists( 'multiple_business_get_youtube_id' ) ):
/**
 ...
 * @return string | bool
 */
function multiple_business_get_youtube_id( $url ) {
...
if( ! function_exists( 'multiple_business_get_the_category' ) ):
/**
* Returns categories after sorting by term id descending
* 
* @since Multiple Business 1.2
* @uses multiple_business_sort_category()
* @return array
*/
function multiple_business_get_the_category( $id = false ){
    $failed = true;
    ...
    if( $failed ){
      ...
      usort( $cat, 'multiple_business_sort_category' );  
    }
...
if( ! function_exists( 'multiple_business_sort_category' ) ):
/**
* Helper function for multiple_business_get_the_category()
*
* @since Multiple Business 1.0.0
*/
function multiple_business_sort_category( $a, $b ){
    return $a->term_id < $b->term_id;
}
endif;

if( ! function_exists( 'multiple_business_explode_string_to_int' ) ):
/**
...
*/
function multiple_business_explode_string_to_int( $str ){
    if( empty( $str ) )
        return false;
...
if( ! function_exists( 'multiple_business_get_menu' ) ):
/**
* Returns wp nav
*
* @since Multiple Business 1.0.0
...
* @return String
*/
function multiple_business_get_menu( $location = 'primary', $echo = true ){
    ...
    switch( $location ){
        ...
        case 'primary':
            $args[ 'fallback_cb' ] = 'multiple_business_primary_menu_fb';
            $args[ 'menu_id' ] = 'primary-menu';
            $args[ 'menu_class' ] = 'primary-menu';
            $menu = wp_nav_menu( apply_filters( 'multiple_business_'.$location.'_menu', $args ) );
        break;

        case 'topheader':
            $args[ 'fallback_cb' ] = 'multiple_business_top_header_menu_fb';
...
if( !function_exists( 'multiple_business_hex2rgba' ) ):
/**
* Convert hexdec color string to rgb(a) string
* @since Multiple Business 1.0.0
*/
function multiple_business_hex2rgba($color, $opacity = false) {
