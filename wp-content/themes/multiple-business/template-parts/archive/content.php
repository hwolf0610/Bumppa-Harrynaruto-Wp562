...
?>
<?php $class = ''; ?>
<?php multiple_business_get_option( 'archive_post_layout' ) == 'grid' ? $class = 'masonry-grid' : $class = 'masonry-grid wrap-post-list'; ?>
<div class="<?php echo esc_attr( $class ); ?>">
	<article id="post-<?php the_ID(); ?>" <?php post_class( 'single-post-wrap' ); ?>>
		<?php $align_class = ''; ?>
		<?php if( multiple_business_get_option( 'archive_post_image_alignment' ) == 'left' ){
			$align_class = 'text-left';
		}elseif( multiple_business_get_option( 'archive_post_image_alignment' ) == 'right' ){
		...
		<?php
		if( multiple_business_get_option( 'archive_post_image' ) == 'thumbnail' ){
			$size = 'thumbnail';
		}elseif( multiple_business_get_option( 'archive_post_image' ) == 'medium'){
			$size = 'medium';
		}elseif( multiple_business_get_option( 'archive_post_image' ) == 'large'){
			$size = 'large';
		}else {
		...
		if( 'post' != get_post_type() && multiple_business_is_search() ){
			$args[ 'dummy' ] = false;
		}

		multiple_business_post_thumbnail( $args );
		?>
			<div class="post-content">
				<?php if( 'post' == get_post_type() ): ?>
					<div class="post-format-outer">
						<span class="post-format">
							<span class="kfi <?php echo esc_attr( multiple_business_get_icon_by_post_format() ); ?>"></span>
						</span>
					</div>
				<?php endif; ?>
				<?php 
